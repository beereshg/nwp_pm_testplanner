...
> **Source**: Co-Design MCP lineage query (2026-07-18) — aliases searched: "Package RAPL", "PKG_RAPL_LIMIT",
> "IA32_PKG_RAPL_LIMIT (MSR 0x610)", "PACKAGE_RAPL_LIMIT (CSR)", "TPMI PL1_CONTROL / PL2_CONTROL (TPMI_ID=0x00)",
> "PKG_POWER_LIMIT", "RAPL PL1/PL2", "Running Average Power Limit".
...
|---|---|---|---|---|---|
| pre-GNR / earlier | no spec access | no spec access | no spec access | no spec access | adapt |
| Wave3_common | Package RAPL exists as package-domain feature with MSR and CSR control; PL1 enabled by default; energy status always supported independent of limit enable | Tests written to MSR/CSR package-domain interfaces valid for this era | "The energy status of all domains is always supported independent of enabling of the domain RAPL limits. By default, package domain limits (PL1 and PL2) are enabled, all other limits are disabled." | [OneSource Wiki](https://docs.intel.com/documents/pm_doc/src/server/Wave3_common/Socket_RAPL/Socket_RAPL.html) | unchanged |
| Wave3_common | Explicit CSR register model: `PCU_CR_PACKAGE_POWER_SKU_UNIT`, `PCU_CR_PACKAGE_ENERGY_STATUS`, `PCU_CR_PACKAGE_RAPL_LIMIT`, `PACKAGE_RAPL_PERF_STATUS` | CSR-based tests valid; TPMI-era tests must adapt | "PCU_CR_PACKAGE_POWER_SKU_UNIT… PCU_CR_PACKAGE_ENERGY_STATUS… PCU_CR_PACKAGE_RAPL_LIMIT… PACKAGE_RAPL_PERF_STATUS" | [OneSource Wiki](https://docs.intel.com/documents/pm_doc/src/server/Wave3_common/Socket_RAPL/Socket_RAPL.html) | unchanged |
| GNR | First generation-specific Socket RAPL feature document; HPM-aware algorithm collateral | Establishes generation-specific HPM flow; older generic tests need adaptation | "Initial version of socket RAPL, 04/11/19" and "socket RAPL for HPM algo added, 04/21/19" | [GNR/SRF](https://docs.intel.com/documents/pm_doc/src/server/GNR/Features/socketRAPL/socket_rapl.html) | adapt |
| GNR | PECI PCS remains part of supported/impacted energy interface set | PECI-PCS based tests still applicable in GNR | "PECI-PCS Index 3, Parameter 0xFF" listed under Socket RAPL impacted interfaces | [GNR/SRF](https://docs.intel.com/documents/pm_doc/src/server/GNR/Features/socketRAPL/socket_rapl.html) | unchanged |
| GNR | TPMI energy/status interface exists alongside MSR/CSR; MSR 0x606/0x610/0x611/0x613/0x614 remain mapped | MSR-era tests not invalidated but must also validate TPMI path from GNR onward | "TPMI: ENERGY_STATUS (GNR onwards)" and mapping table "PACKAGE_POWER_SKU_UNIT,606h… PACKAGE_RAPL_LIMIT,610h… PACKAGE_ENERGY_STATUS,611h…" | [GNR/SRF](https://docs.intel.com/documents/pm_doc/src/server/GNR/Features/socketRAPL/socket_rapl.html) | adapt |
| GNR | Interface evolution: PCS command clarification, HPM message clarification, anti-windup, PL2 config, energy filtering, no-DCF-lower-power-limits | Algorithm bars must adapt per revision deltas | "Added integral anti-windup mechanism and PID equations" / "Clarified Socket RAPL PL2 PID power limit configuration" / "Updated description of energy_filtering_enable bit" | [GNR/SRF](https://docs.intel.com/documents/pm_doc/src/server/GNR/Features/socketRAPL/socket_rapl.html) | adapt |
| DMR | Socket RAPL scope expands to hierarchical package-level control across all CBB and IMH dies | Topology-aware tests must adapt from monolithic to hierarchical-socket behavior | "Socket RAPL controls the power balancing/distribution across all components physically located within the socket including all the CBB and IMH dies." | [DMR CBB](https://docs.intel.com/documents/pm_doc/src/server/dmr/pm%20features/dmr_rapl_simplification.html) | adapt |
| DMR | Controller consolidation: power-limit arrays / PID controller instances minimized to **2 instances** | Tests assuming more controllers must adapt | "the socket rapl domain structure(s) like power limit input array, power budget, PID controller instances are minimized two instances as expected in DMR." | [OneSource Wiki](https://docs.intel.com/documents/pm_doc/src/server/dmr/pm%20features/dmr_rapl_simplification.html) | adapt |
| DMR | **MSR sampling + PCS input sampling deprecated**; DMR samples CSR only (GNR sampled MSR+CSR+PCS) | MSR/PCS active-control tests **invalidated** for DMR; move to negative/deprecation checks | "Sampling of MSR registers and PCS input will be deprecated in DMR." | [OneSource Wiki](https://docs.intel.com/documents/pm_doc/src/server/dmr/pm%20features/dmr_rapl_simplification.html) | **invalidates** |
| DMR | **RAPL MSR registers deprecated** (0x606, 0x610, 0x611 + platform/DRAM equivalents); PrimeCode does not initialize; writes no-op | MSR functional programming tests **invalidated**; replace with negative tests | "associated rapl msr registers (_SKU_UNIT, PACKAGE/PLATFORM/DRAM _POWER_INFO, …_RAPL_LIMIT, …_ENERGY_STATUS) are **deprecated**. No initialization … any write(s) … will not have any action from PrimeCode." | [OneSource Wiki](https://docs.intel.com/documents/primecode/primecode_one/firmware%20architecture/ip%20drivers%20and%20libraries/rapl_dmr.html) | **invalidates** |
| DMR | TPMI and CSR coexist; only `PL_INFO` and `POWER_UNIT` updated in TPMI at init; rest hold HW defaults | TPMI readback tests must adapt: not all state PrimeCode-populated at init | "For TPMI, only PL_INFO (sku definition) and POWER_UNIT are updated to same value as CSR. Rest of registers will hold HW default values." | [OneSource Wiki](https://docs.intel.com/documents/primecode/primecode_one/firmware%20architecture/ip%20drivers%20and%20libraries/rapl_dmr.html) | adapt |
| DMR | BIOS maps Socket RAPL knobs to TPMI `POWER_LIMIT_CONTROL_1/2` and CSR `PACKAGE_RAPL_LIMIT_CFG`; RAPL MSR deprecated | BIOS/MSR era tests must adapt to TPMI/CSR knob mapping | "PL1 Limit, POWER_LIMIT_CONTROL_1.PWR_LIM_EN, PACKAGE_RAPL_LIMIT_CFG.PKG_PWR_LIM_1_EN" and "NOTE: RAPL MSR is deprecated. BIOS HSD 14018460453" | [OakStream CPUPM FAS](https://docs.intel.com/documents/System-firmware-bios/Domain/ServerCpuPm/Dmr/DMR/Oakstream_CPUPM_FAS.html) | adapt |
| DMR | PrimeCode revisions: FastRAPL details, reset phases/events, actuator feedback, PLR backward compat, PL1 always enabled on TPMI, cold TDP | Existing tests need adaptation for reset sequencing, PLR compat, FastRAPL timing, always-enabled PL1 | "PL1 will always stay enabled on TPMI" / "Adding integral clipping to Fast RAPL to match HAS" / "Adding Details about Cold TDP behavior" | [PrimeCode RAPL DMR](https://docs.intel.com/documents/primecode/primecode_one/firmware%20architecture/ip%20drivers%20and%20libraries/rapl_dmr.html) | adapt |
| DMR | HPM messaging explicit root/leaf: IMH-P root sends `HPM_RAPL_PERF_LIMIT` to IMH-S and CBBs; CBBs return `HPM_LEAF_PERF_STATUS` | Earlier local-only package tests must adapt to cross-die HPM distribution | "The root socket rapl instance in IMH0 would send HPM_RAPL_PERF_LIMIT to leaf socket rapl instances running in IMH1 and all the leaf socket rapl instances running in the CBB dielets." | [DMR CBB](https://docs.intel.com/documents/pm_doc/src/server/dmr/pm%20features/dmr_rapl_simplification.html) | adapt |
| NWP | No direct Socket RAPL feature delta document surfaced; NWP uses NWP-specific IMH (NIO) collateral where IMH-side behavior differs | no spec access for bounded NWP Socket RAPL clause | no spec access | NWP wiki / NWP IMH | adapt |

### Lineage Summary (DMR → NWP tail)

DMR is the key interface break: MSR/PCS active-control paths deprecated, CSR becomes the firmware-control path, TPMI becomes the BIOS/software-facing path, and Socket RAPL becomes explicitly hierarchical (IMH↔CBB HPM distribution). NWP carries forward DMR Socket RAPL behavior with topology change: single NIO replaces dual IMH (1 PrimeCode root, 2 CBBs instead of 4). No bounded NWP Socket RAPL delta clause was retrieved from Co-Design specs — safe working assumption is DMR carry-forward.
