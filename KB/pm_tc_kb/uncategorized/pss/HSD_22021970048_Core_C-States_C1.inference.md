...
- Test Result: [22022027541 - [PSS][CORE_CSTATES] C1 Residency Counter](https://hsdes.intel.com/appstore/article/#/22022027541)

## KB References
- KB Article: [KB/pm_features/core_c_states/c1.md](../../../KB/pm_features/core_c_states/c1.md)

## Model Response

## Refined Intent
Verify C1 residency counter (MSR 0x3FD) increments when cores enter C1 state via MWAIT using Intel Idle driver or Solar tool. This flow enables cross-product coverage with CXL VTC domain.

## Refined Test Steps
Pre-Conditions:
  - C-states enabled in BIOS (C1 at minimum)
...
Step 1 — Inject MWaits:
  Use Intel Idle Driver or Solar tool to inject MWaits with C1 hint.

Step 2 — Verify BIOS ACPI enumeration:
  Confirm C-states properly enumerated via ACPI.

Step 3 — Verify C1 residency counter:
  Read MSR 0x3FD — verify counter is incrementing.
  Delta should be > 0 and proportional to idle time.

Pass/Fail Criteria:
  PASS: CC1/CC6 residency counter (MSR 0x3FD) is incrementing after C1 entry
  FAIL: Residency counter not incrementing

HAS/MAS References:
  - DMR CBB PM HAS — Core C1 Residency: https://docs.intel.com/documents/pm_doc/src/server/DMR/IP_PM_Features/CBB/DMR_CBB_PM.html

### NWP Project Relevance
**Test Classification:** Regression (DMR-inherited)
**Feature Status:** Expected to work
**Test Purpose:** Verify C1 residency counter (MSR 0x3FD) increments when cores enter C1 state via MWAIT using Intel Idle driver or Solar tool. This flow enables cross-product coverage with CXL VTC domain.
...
3. Step 3 — Verify C1 residency counter:
...
| 0x660 C1_RESIDENCY | Yes | Register access |
