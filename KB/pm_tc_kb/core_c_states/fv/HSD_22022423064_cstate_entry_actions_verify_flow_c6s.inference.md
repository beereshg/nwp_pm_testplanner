...
|------|--------|----------------------|-------------------|
| 1 | Set all cores idle (via MWAIT or sleep); observe C6S entry via PythonSV. | All cores enter target C-state; state machine registers show {cs} state. | Cores stuck in C1 or shallower state; entry timeout. |
...
- Exit from C6S < spec latency; core returns to C0 cleanly.
- NWP: PC6 residency = 0 (PkgC6 ZBB).

### Pass / Fail Criteria

- **PASS**: All cores enter C6S and counters increment; exit latency within spec; no MCA.

- **FAIL**: C6S not entered; counter stuck; exit timeout; MCA.
...
- HSD TC: https://hsdes.intel.com/appstore/article-one/#/22022423064