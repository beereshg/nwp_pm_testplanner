...
Verify the C-state exit flow for C6S-P on NWP. When a core receives a wake event (interrupt, IPI, or timer), pCode/Acode executes the C6S-P exit sequence: restore frequency/voltage, ungating clocks, re-enabling caches, and transitioning back to C0. This TC confirms exit actions complete correctly.
...
|------|--------|----------------------|-------------------|
| 1 | Drive cores into C6S-P (via MWAIT or idle); confirm entry. | Cores in {cs} state; residency counter incrementing. | Entry fails; cannot test exit. |
...
- No MCA during C6S-P exit.
- Exit latency within spec for 2 CBBs × 48 cores.
- Post-exit: `sv.socket0.cbb{i}.core[c].CLOCK_CST_CONFIG_CONTROL` = C0 for all cores.
- No residual C6S-P state bit set after full exit.

### Pass / Fail Criteria

- **PASS**: All cores exit C6S-P within spec latency; return to C0 cleanly; no MCA.

- **FAIL**: Exit timeout; core stuck in C6S-P post-wake; MCA.
...
- HSD TC: https://hsdes.intel.com/appstore/article-one/#/22022423076