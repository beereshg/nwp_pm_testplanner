# Deep Analysis: Corelating timings_silicon_vp
...
| **Sub-Feature** | Correlating timings -- PEM_STATUS bit set latency via Virtual Platform (Simics) |
...
This test verifies that after introducing a throttle event, the **time taken to set PEM_STATUS bit** matches the configured TW (Time Window / EWMA window) setting within +/-10% tolerance, measured via the **Virtual Platform (Simics)** interface. Tests PEM event response latency accuracy.
...
Validates the **Correlating Timings** scenario defined in [TCD 22022421012 -- Corelating timings](https://hsdes.intel.com/appstore/article-one/#/22022421012), using the **Virtual Platform (Simics)** readback interface. Verifies PEM_STATUS bit set latency matches the TW (Time Window) setting. Environment: NWP Simics VP (PSS environment).

> **Original HSD flow preserved:** 1. Enable PEM through TPMI. 2. Introduce throttling event. 3. Check time taken to set respective bit in PEM_STATUS via VP interface -- should be ~ TW setting. Configuration: BMC setup, LPF ramp calculation from thermal team.
...
| 5 | Poll PEM_STATUS via **Virtual Platform (Simics)**: PythonSV TPMI model PEM_STATUS read in VP | PEM_STATUS excursion bit transitions from 0 to 1 | Bit never set |
...
- **PASS**: `T_set = TW_setting +/- 10%` AND PEM_STATUS bit correctly identifies the excursion source AND VP interface returns valid data
- **FAIL**: T_set deviates > 10% from TW_setting OR PEM_STATUS bit not set after throttle OR VP readback fails
