...
| **Segment** | FV |
| **TPF** | Module C-States (Core→Module) |
| **TCD** | CStates MC6 residency counter |
| **Status** | open |
...
|------|--------|----------------------|-------------------|
| 1 | Set all cores idle (via MWAIT or sleep); observe MC6 entry via PythonSV. | All cores enter target C-state; state machine registers show {cs} state. | Cores stuck in C1 or shallower state; entry timeout. |
...
- Exit from MC6 < spec latency; core returns to C0 cleanly.
- NWP: PC6 residency = 0 (PkgC6 ZBB).

### Pass / Fail Criteria

- **PASS**: All cores enter MC6 and counters increment; exit latency within spec; no MCA.

- **FAIL**: MC6 not entered; counter stuck; exit timeout; MCA.
...
- HSD TC: https://hsdes.intel.com/appstore/article-one/#/22022423069