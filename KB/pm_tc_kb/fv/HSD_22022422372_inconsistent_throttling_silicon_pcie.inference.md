# Deep Analysis: Inconsistent Throttling_silicon_PCIE
...
| **Sub-Feature** | Inconsistent throttling (gaps) via PCIe (PMT/OOB) |
...
Tests PEM accuracy with **gaps in throttle events** (5-second gaps between throttle periods). PEM must correctly accumulate only the actual throttle time (not the gaps). Formula: `PEM counter (ms) = total induced throttle time +/- 10%`. Readback via **PCIe (PMT/OOB)**.
...
| 5 | After all cycles complete, read PEM counters via **PCIe (PMT/OOB)**: PMT PEM counter read via PCIe OOB path | PEM counter value returned (ms) | Interface read failure |
...
- **PASS**: `PEM counter = total throttle-ON time +/- 10%` AND gap time NOT counted AND correct PEM_STATUS bits set AND PCIe readback returns valid data
- **FAIL**: PEM counter includes gap time OR deviates > 10% from actual throttle-ON time OR PEM_STATUS bits not set OR PCIe readback fails
