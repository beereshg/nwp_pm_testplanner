# Deep Analysis: Consistent throttling_silicon_PCIE
...
| **Sub-Feature** | Consistent throttling via PCIe (PMT/OOB) |
...
PEM consistent throttling validates that 1 minute of continuous, uninterrupted throttle is accurately captured by PEM counters read via the **PCIe (PMT/OOB)** interface. PEM is functional on NWP. Test formula: `Total time = LPF ramp + PEM counter captured time ~ 1 minute`.
...
Validates the **Consistent Throttling** scenario defined in [TCD 22022421010 -- Consistent Throttling](https://hsdes.intel.com/appstore/article-one/#/22022421010), using the **PCIe (PMT/OOB)** readback interface. Verifies that PEM counters accurately capture total throttle duration (LPF ramp + sustained throttle ~ 60 s) when throttle is continuous (no gaps). Environment: NWP post-silicon with PCIe OOB/PMT access.

> **Original HSD flow preserved:** 1. Enable PEM through TPMI. 2. Enforce throttling for 1 minute contiguously without gaps. 3. Check respective counters for total time of throttling via PCIe interface. Configuration: BMC setup, LPF ramp calculation from thermal team.
...
| Item | Requirement |
|------|-------------|
| Platform | NWP post-silicon with PCIe OOB/PMT access |
| BIOS knobs | Default (PEM always active when RAPL enabled) |
| BMC / OOB | PCIe OOB telemetry path configured |
...
| 6 | Read PEM counters via **PCIe (PMT/OOB)**: PMT telemetry via PCIe OOB path | PEM counter value returned | Interface read failure |
...
- **PASS**: `LPF ramp + PEM counter = 60 s ± 10%` AND correct PEM_STATUS bits set AND PCIe interface returns valid PEM data
- **FAIL**: PEM counter total deviates > 10% from 60 s OR PEM_STATUS bits not set for active limiter OR PCIe readback path returns no data / error
