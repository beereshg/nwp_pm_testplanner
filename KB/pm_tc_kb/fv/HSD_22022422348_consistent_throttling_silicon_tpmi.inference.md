# Deep Analysis: Consistent throttling_silicon_TPMI
...
| **Sub-Feature** | Consistent throttling via TPMI (in-band MMIO) |
...
PEM consistent throttling validates that 1 minute of continuous, uninterrupted throttle is accurately captured by PEM counters read via the **TPMI (in-band MMIO)** interface. PEM is functional on NWP. Test formula: `Total time = LPF ramp + PEM counter captured time ~ 1 minute`.
...
Validates the **Consistent Throttling** scenario defined in [TCD 22022421010 -- Consistent Throttling](https://hsdes.intel.com/appstore/article-one/#/22022421010), using the **TPMI (in-band MMIO)** readback interface. Verifies that PEM counters accurately capture total throttle duration (LPF ramp + sustained throttle ~ 60 s) when throttle is continuous (no gaps). Environment: NWP post-silicon with PythonSV access.

> **Original HSD flow preserved:** 1. Enable PEM through TPMI. 2. Enforce throttling for 1 minute contiguously without gaps. 3. Check respective counters for total time of throttling via TPMI interface. Configuration: BMC setup, LPF ramp calculation from thermal team.
...
| 6 | Read PEM counters via **TPMI (in-band MMIO)**: `sv.socket0.cbb{0,1}.base.tpmi.pem_status.show()` | PEM counter value returned | Interface read failure |
...
- **PASS**: `LPF ramp + PEM counter = 60 s ± 10%` AND correct PEM_STATUS bits set AND TPMI interface returns valid PEM data
- **FAIL**: PEM counter total deviates > 10% from 60 s OR PEM_STATUS bits not set for active limiter OR TPMI readback path returns no data / error
