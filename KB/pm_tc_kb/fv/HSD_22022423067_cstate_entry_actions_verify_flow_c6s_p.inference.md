...
Verify the C-state entry flow for C6S-P on NWP. When a core becomes idle, pCode/Acode executes the C6S-P entry sequence: voltage/frequency reduction, clock gate, firmware handshake, and state machine progression. This TC confirms the entry actions complete correctly with expected register state at the end of entry.
...
|------|--------|----------------------|-------------------|
| 1 | Set all cores idle (via MWAIT or sleep); observe C6S-P entry via PythonSV. | All cores enter target C-state; state machine registers show {cs} state. | Cores stuck in C1 or shallower state; entry timeout. |
...
- No MCA during C6S-P entry/exit.
...
- Exit from C6S-P < spec latency; core returns to C0 cleanly.
- NWP: PC6 residency = 0 (PkgC6 ZBB).

### Pass / Fail Criteria

- **PASS**: All cores enter C6S-P and counters increment; exit latency within spec; no MCA.

- **FAIL**: C6S-P not entered; counter stuck; exit timeout; MCA.
...
- HSD TC: https://hsdes.intel.com/appstore/article-one/#/22022423067