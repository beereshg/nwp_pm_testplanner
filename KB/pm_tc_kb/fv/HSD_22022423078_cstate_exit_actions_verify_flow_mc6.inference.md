...
Verify the C-state exit flow for MC6 on NWP. When a core receives a wake event (interrupt, IPI, or timer), pCode/Acode executes the MC6 exit sequence: restore frequency/voltage, ungating clocks, re-enabling caches, and transitioning back to C0. This TC confirms exit actions complete correctly.
...
|------|--------|----------------------|-------------------|
| 1 | Drive cores into MC6 (via MWAIT or idle); confirm entry. | Cores in {cs} state; residency counter incrementing. | Entry fails; cannot test exit. |
...
- No MCA during MC6 exit.
- Exit latency within spec for 2 CBBs × 48 cores.
- Post-exit: `sv.socket0.cbb{i}.core[c].CLOCK_CST_CONFIG_CONTROL` = C0 for all cores.
- No residual MC6 state bit set after full exit.

### Pass / Fail Criteria

- **PASS**: All cores exit MC6 within spec latency; return to C0 cleanly; no MCA.

- **FAIL**: Exit timeout; core stuck in MC6 post-wake; MCA.
...
- HSD TC: https://hsdes.intel.com/appstore/article-one/#/22022423078