...
- Test Case: [22021970176 - PEGA-driven FabricGV](https://hsdes.intel.com/appstore/article/#/22021970176)
- Test Result: [22022027602 - [PSS][DVFS] PEGA-driven FabricGV](https://hsdes.intel.com/appstore/article/#/22022027602)
...
## Refined Intent
Verify PEGA-driven fabric GV transitions via U2P mailbox. NWP: UFS/FabricGV is ZBB'd — mesh/CAB fixed at 2 GHz. PEGA FabricGV commands should have no effect.

## Refined Test Steps
Pre-Conditions:
  - NWP: FabricGV ZBB'd — negative validation

Step 1 — Inject PEGA FabricGV command:
  Write PEGA CMD0/CMD1/CMD2 for meshgv via U2P mailbox.
  On NWP: expect no mesh frequency change.

Step 2 — Verify mesh stays at 2 GHz:
  Read UFS_STATUS — verify unchanged.

Pass/Fail Criteria:
  PASS (NWP): PEGA FabricGV has no effect — mesh stays at 2 GHz (ZBB'd)
  FAIL: Mesh frequency changes on NWP

HAS/MAS References:
  - DMR CBB PEGA HAS — FabricGV: https://docs.intel.com/documents/pm_doc/src/DMR_CBB/Features/PEGA/PEGA.html
...
**Test Purpose:** Verify PEGA-driven fabric GV transitions via U2P mailbox. NWP: UFS/FabricGV is ZBB'd — mesh/CAB fixed at 2 GHz. PEGA FabricGV commands should have no effect.
**Negative Test Aspect:** None
**NWP Delta:** Topology differences from DMR (2 CBB + 1 NIO); same Fabric DVFS behavior expected

## Section A: Critical Execution Path
1. Step 1 — Inject PEGA FabricGV command:
2. Step 2 — Verify mesh stays at 2 GHz:
