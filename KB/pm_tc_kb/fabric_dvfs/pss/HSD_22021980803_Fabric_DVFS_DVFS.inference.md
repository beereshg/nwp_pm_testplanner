...
## Metadata
- HSD ID: 22021980803
- Title: TPMI register verification - IMH MEM
- Feature: Fabric DVFS
- Sub Feature: DVFS
...
- Test Cycle: Newport Product.trunk.pss_0p5.pss.val.NWP_NIO-HSLE
- NWP Scope: Runnable_On_N-1

## HSD Hierarchy
- Test Case Definition: [22021980310 - TPMI Registers](https://hsdes.intel.com/appstore/article/#/22021980310)
- Test Case: [22021980803 - TPMI register verification - IMH MEM](https://hsdes.intel.com/appstore/article/#/22021980803)
- Test Result: [22022027698 - [PSS][DVFS] TPMI register verification - IMH MEM](https://hsdes.intel.com/appstore/article/#/22022027698)
...
## Refined Intent
Verify default values across all UFS TPMI registers for IMH MEM. NWP: UFS is ZBB'd — registers should show fixed state.

## Refined Test Steps
Pre-Conditions:
  - NWP: UFS ZBB'd — negative validation

Step 1 — Read all IMH MEM UFS TPMI registers.

Step 2 — Verify fixed/default values reflecting ZBB'd state.

Pass/Fail Criteria:
  PASS (NWP): IMH MEM UFS registers show fixed state
  FAIL: Dynamic UFS present
...
**Test Purpose:** Verify default values across all UFS TPMI registers for IMH MEM. NWP: UFS is ZBB'd — registers should show fixed state.
**Negative Test Aspect:** None
**NWP Delta:** Topology differences from DMR (2 CBB + 1 NIO); same Fabric DVFS behavior expected

## Section A: Critical Execution Path
1. Step 1 — Read all IMH MEM UFS TPMI registers.
2. Step 2 — Verify fixed/default values reflecting ZBB'd state.
