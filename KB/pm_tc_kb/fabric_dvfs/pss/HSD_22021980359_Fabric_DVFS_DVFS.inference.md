...
## Metadata
- HSD ID: 22021980359
- Title: TPMI register verification - IMH IO
- Feature: Fabric DVFS
- Sub Feature: DVFS
...
- Test Case: [22021980359 - TPMI register verification - IMH IO](https://hsdes.intel.com/appstore/article/#/22021980359)
- Test Result: [22022027610 - [PSS][DVFS] TPMI register verification - IMH IO](https://hsdes.intel.com/appstore/article/#/22022027610)
...
## Refined Intent
Verify default values across all UFS TPMI registers for IMH IO. NWP: UFS is ZBB'd — registers should show fixed state.

## Refined Test Steps
Pre-Conditions:
  - NWP: UFS ZBB'd — negative validation

Step 1 — Read all IMH IO UFS TPMI registers.

Step 2 — Verify fixed/default values reflecting ZBB'd state.

Pass/Fail Criteria:
  PASS (NWP): IMH IO UFS registers show fixed state
...
**Test Purpose:** Verify default values across all UFS TPMI registers for IMH IO. NWP: UFS is ZBB'd — registers should show fixed state.
**Negative Test Aspect:** None
**NWP Delta:** Topology differences from DMR (2 CBB + 1 NIO); same Fabric DVFS behavior expected

## Section A: Critical Execution Path
1. Step 1 — Read all IMH IO UFS TPMI registers.
