...
- Test Case: [22021970106 - PEM - Socket RAPL PL1](https://hsdes.intel.com/appstore/article/#/22021970106)
- Test Result: [22022027533 - [PSS][SOCKET_RAPL] PEM - Socket RAPL PL1](https://hsdes.intel.com/appstore/article/#/22022027533)
...
## Refined Intent
Verify whether RAPL throttles IPs when PL1 power limit is exceeded. RAPL algorithm is implemented in Primecode and enforces power consumed by SoC to within PL1/PL2. In NWP only Socket RAPL is implemented (DRAM RAPL and Platform RAPL are ZBB). BIOS, OS and FW are required for simulating end-to-end RAPL flow. MBVR Xtors provide power data as input to RAPL algorithm.

## Refined Test Steps
Pre-Conditions:
  - No special configuration required, boot with default fuses
  - BIOS knobs: PL1 enable, PL1 power limit, PL1 time window
  - Ingredients: BIOS, OS, Pcode

Step 1 — Request >Pn ratios:
  Request >Pn ratios on core/mesh on CBB and IMH dies.

Step 2 — Set Socket RAPL PL1 to low value:
  Set via BIOS (initial values) or TPMI (runtime) to a low value.
  Use Punit register: package_rapl_limit.pkg_pwr_lim_1 and pkg_pwr_lim_1_en.
  Ensure PL1 is consumed by Pcudata/TPMI.

Step 3 — Read PEM Status:
  Read socket RAPL PEM Status registers:
  sv.socket0.imh0.punit.ptpcfsms.ptpcfsms.pem_status.pkg_pl1_oob
  sv.socket0.imh0.punit.ptpcfsms.ptpcfsms.pem_status.pkg_pl1_csr
  sv.socket0.imh0.punit.ptpcfsms.ptpcfsms.pem_status.pkg_pl1_inband
  Optional: check socket power consumed and core/mesh ratios.

Step 4 — Repeat with other PL1 values:
  Program different PL1 values and re-check PEM status.

Pass/Fail Criteria:
  PASS: RAPL perf status increments and core/mesh ratios throttle in response to PL1 violation
  FAIL: No PEM status change or no throttling on PL1 violation

HAS/MAS References:
  - DMR RAPL Simplification HAS — PL1: https://docs.intel.com/documents/pm_doc/src/server/DMR/PM%20Features/DMR_RAPL_Simplification.html
...
**Test Purpose:** Verify whether RAPL throttles IPs when PL1 power limit is exceeded. RAPL algorithm is implemented in Primecode and enforces power consumed by SoC to within PL1/PL2. In NWP only Socket RAPL is implemen
...
2. Step 2 — Set Socket RAPL PL1 to low value:
3. Step 3 — Read PEM Status:
4. Step 4 — Repeat with other PL1 values:
...
| TPMI: package_rapl_limit (PL1) | Yes | TPMI interface |
