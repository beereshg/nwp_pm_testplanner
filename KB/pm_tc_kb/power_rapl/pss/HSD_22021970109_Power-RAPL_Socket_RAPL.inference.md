...
- Test Case: [22021970109 - PLR- Socket RAPL PL1](https://hsdes.intel.com/appstore/article/#/22021970109)
- Test Result: [22022027575 - [PSS][SOCKET_RAPL] PLR- Socket RAPL PL1](https://hsdes.intel.com/appstore/article/#/22022027575)
...
Step 4 — Verify cores are throttled:
  Confirm cores are throttled based on the RAPL algorithm.
  Verify throttling matches expected PL1 enforcement behavior.

Step 5 — Release PL1 constraint:
  Restore PL1 to original value or raise above current power.
  Verify PLR bits clear and cores recover frequency.

Pass/Fail Criteria:
  PASS: Cores throttled based on RAPL algorithm, PLR bits set accordingly when PL1 exceeded
  FAIL: PLR bits not set when PL1 exceeded, or cores not throttled

HAS/MAS References:
  - DMR RAPL Simplification HAS — PL1 PLR: https://docs.intel.com/documents/pm_doc/src/server/DMR/PM%20Features/DMR_RAPL_Simplification.html#register-programming
...
**Test Purpose:** Verify relevant PLR (Perf Limit Reasons) bits are set correctly when power consumed exceeds PL1 limits. RAPL power limits and time windows are programmed by BIOS via CSR; OS can reprogram via TPMI. Pr
**Negative Test Aspect:** None
**NWP Delta:** Topology differences from DMR (2 CBB + 1 NIO); same Power/RAPL behavior expected

## Section A: Critical Execution Path
1. Step 1 — Exceed RAPL PL1 limits:
2. Step 2 — Verify PLR bits are set:
3. Step 3 — Verify SST TPMI default values:
4. Step 4 — Verify cores are throttled:
5. Step 5 — Release PL1 constraint:
