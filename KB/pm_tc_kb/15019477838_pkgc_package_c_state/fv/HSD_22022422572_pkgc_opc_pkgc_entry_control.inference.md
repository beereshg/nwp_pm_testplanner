# HSD 22022422572: OPC_PKGC_ENTRY_CONTROL

| Field | Value |
|-------|-------|
| **HSD ID** | [22022422572](https://hsdes.intel.com/appstore/article/#/22022422572) |
| **Segment** | FV |
| **Feature** | Package C-States |
| **Sub-Feature** | PkgC x PECI |
...
PECI OPC can block PC6 entry. When set to 1, expect 0 PC6 residency.
...
**Disposition: Modify (verify blocked)**
...
**Verify OPC_PKGC_ENTRY_CONTROL register exists but has no effect (PC6 already blocked by fuse)**
...
1. **Disposition**: Modify (verify blocked)
2. **Negative test**: Verify OPC_PKGC_ENTRY_CONTROL register exists but has no effect (PC6 already blocked by fuse)
